...
export async function emitirOtp(email, purpose) {
    ...
    if (!user || (purpose === 'verify' && user.isVerified)) return;
    if (!process.env.RESEND_API_KEY || !process.env.EMAIL_REMETENTE) throw new Error('Envio de e-mail indisponível.');
...
export async function consumirOtp(email, purpose, code, password) {
    ...
    await mongoose.connection.transaction(async (session) => {
        ...
        const update = purpose === 'verify' ? { $set: { isVerified: true } }
            : { $set: { password: newHash, isVerified: true }, $inc: { tokenVersion: 1 } };
